		...
		compiler.plugin("compilation", (compilation) => {
          ...
          chunk.files
              ...
              const debugInfo = getDebugInfo();
							...
							const detectedGlobal = `(typeof global === 'object' ? global : self)`;

              let injectedCode = options.multiple ?
                `${detectedGlobal}.${options.variableName} = ${detectedGlobal}.${options.variableName} || [];
                ${detectedGlobal}.${options.variableName}.push('${finalStr}');` :
                `${detectedGlobal}.${options.variableName} = '${finalStr}';`;
