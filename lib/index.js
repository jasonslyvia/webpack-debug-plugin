		...
		compiler.plugin("compilation", (compilation) => {
          ...
          chunk.files
              ...
              const debugInfo = getDebugInfo();
							...
							const detectGlobal = `const topRef = typeof global === 'object' ? global : self;`;

              let injectedCode = options.multiple ?
                `${detectGlobal}topRef.${options.variableName} = topRef.${options.variableName} || [];
                topRef.${options.variableName}.push('${finalStr}');` :
                `${detectGlobal}topRef.${options.variableName} = '${finalStr}';`;
